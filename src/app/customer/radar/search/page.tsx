...
const CAPABILITIES: Array<{
  ...
  { key: "enrich", label: "信息补全", description: "自动补全网站、地区、行业和规模信息。", icon: Sparkles, alwaysActive: true },
  { key: "contacts", label: "联系人识别", description: "识别决策链角色与潜在触达路径。", icon: Target, alwaysActive: true },
];
...
export default function RadarSearchPage() {
  ...
  const toggleAutomation = async (mode: "pause" | "resume") => {
    ...
    try {
      ...
      toast.success(mode === "pause" ? "已暂停自动搜索" : "已恢复自动搜索");
      await loadData();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-xl border border-[var(--ci-border)] bg-[#FFFFFF] p-6 shadow-[var(--ci-shadow-soft)]">
        <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-[rgba(79,141,246,0.18)] bg-[rgba(79,141,246,0.08)] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--ci-accent)]"><Radar size={12} />智能搜索</div>
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-[#0B1B2B]">自动搜索</h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-500">系统根据目标客户画像自动发现潜在客户、补全关键信息并筛选推荐。您只需确认画像方向、查看推荐结果，然后推进跟进。</p>
            <div className="mt-5 flex flex-wrap gap-3 text-xs">
              ...
              <Pill label="可用信息渠道" value={sourceSummary.total ? `${sourceSummary.enabled}/${sourceSummary.total}` : "初始化中"} />
            </div>
          </div>
...
<SectionHeader
  eyebrow="目标客户范围"
  title="当前画像与搜索方向"
  description="系统将根据以下客户画像，持续发现匹配的潜在客户。"
/>
          {targetingSpec ? (
        ...
        <section className="rounded-xl border border-[var(--ci-border)] bg-[#FFFFFF] p-6">
          <SectionHeader eyebrow="自动搜索能力" title="系统覆盖的搜索范围" description="系统会自动从多个公开渠道发现公司、补全信息，并筛选出值得跟进的对象。" />
                ...
                {sourceCoverage.map((item) => (
                  <div key={item.key} className="rounded-xl border border-[var(--ci-border)] bg-[var(--ci-surface-muted)] p-4">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
                      <span className="rounded-full bg-white px-2.5 py-1">可用 {item.healthy}</span>
...
<SectionHeader eyebrow="优化搜索方向" title="补充筛选条件" description="根据您的业务判断补充优先级，让后续推荐更准确。" compact />
...
<SectionHeader eyebrow="最近活动" title="最近发生了什么" description="查看系统最近完成的搜索、补全和推荐动作。" />
        ...
        <section className="rounded-xl border border-[var(--ci-border)] bg-[#FFFFFF] p-6">
          <SectionHeader eyebrow="自动搜索计划" title="当前正在运行的搜索任务" description="查看系统自动搜索的运行状况和配置。" />
          {profiles.length ? (
          ...
          {profiles.length ? (
            ...
            <EmptyCard title="暂无自动搜索计划" description="您可以直接按当前画像启动一轮搜索，也可以稍后设置自动搜索计划。" />
          )}
<div className="mt-4 rounded-xl border border-dashed border-[var(--ci-accent)]/35 bg-[var(--ci-surface-strong)] px-4 py-3 text-sm text-slate-600">系统正在从多个公开信息渠道持续发现潜在客户。您只需关注推荐结果，决定哪些值得跟进。</div>
        </section>
      </div>
    </div>
  );
